...
GRPO_ARGS=(
   ...
   --use-kl-loss
   # 0.001 = light regularization, prevents unbounded KL drift while allowing improvement
   # (0.01 was too strong and hurt eval, 0 worked but risked instability)
   --kl-loss-coef 0.001
   --kl-loss-type low_var_kl
   # Keep entropy at 0 - the successful run showed natural entropy decrease (0.25→0.10)
   # Adding entropy bonus can interfere with this natural convergence
   --entropy-coef 0.0
   --eps-clip 0.2
   --eps-clip-high 0.28
)
...
WANDB_ARGS=(
   ...
   --wandb-group QWEN-3-30B-A3B-2-nodes-kl-$(date +%Y%m%d-%H%M%S)
